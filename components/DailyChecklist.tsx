...
export default function DailyChecklist() {
  ...
  // On mount: compute today's key and load saved checks
  useEffect(() => {
  ...
  // Whenever checked changes (after mount), write to localStorage
  useEffect(() => {
  ...
  const allDone = mounted && checked.size === ALL_IDS.length;
  ...
  return (
    <div className="card">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-3">
          <span className="text-xs font-medium text-slate-500">
            {mounted ? checked.size : 0}/{ALL_IDS.length} done
          </span>
      ...
      <div className="mb-3 h-4">
        {allDone && (
          <p className="text-xs text-green-600 font-medium">🎉 All done for today!</p>
        )}
      </div>
